...
import { getSession, getUserPackages, getPackageByQR, getUserById } from '../../lib/storage';
...
const CustomerDashboard = () => {
  ...
  // Load packages assigned to the customer
  useEffect(() => {
    if (session?.userId) {
  ...
  const handleScan = (qrData: string) => {
    ...
    // Check if customer has access to this package
    if (session && pkg.customerId !== session.userId) {
      setScanError('Access denied - This package is not for you');
      return;
    }
          ...
          {!showScanner ? (
            <div className="text-center py-8">
              ...
              <p className="mb-6 text-muted-foreground">
                Scan a package QR code to view details of your delivery
              </p>
              <button
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-background p-4 rounded-lg">
                    ...
                    <p className="text-sm">
                      ...
                      {getUserById(scannedPackage.deliveryAgentId)?.name || 'Unknown'}
                    </p>
                    
                    <div className="mt-4 text-sm text-muted-foreground">
                      {scannedPackage.status === 'pending' && (
                        <p>Your package is being prepared for delivery.</p>
                      )}
        ...
        <section className="bg-card p-6 rounded-xl shadow-sm border border-border">
          ...
          {packages.length === 0 ? (
            ...
            <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2">
              {packages.map((pkg) => {
                const agent = getUserById(pkg.deliveryAgentId);
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-sm text-muted-foreground">
                      <p>
                        {pkg.status === 'pending' && 'Your package is being prepared for delivery.'}
                        {pkg.status === 'in-transit' && 'Your package is on its way to you.'}
                        {pkg.status === 'delivered' && 'Your package has been delivered.'}
                      </p>
                      <p className="mt-1">
